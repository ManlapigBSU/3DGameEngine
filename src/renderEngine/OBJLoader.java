...
public class OBJLoader {
    ...
    private static void processVertex(String[] vertexData, List<Integer> indices, List<Vector2f> textures, List<Vector3f> normals, float[] textureArray, float[] normalsArray) {
        // Process vertex position (always present)
        int currentVertexPointer = Integer.parseInt(vertexData[0]) - 1;
        indices.add(currentVertexPointer);

        // Process texture coordinate (if present)
        if (vertexData.length > 1 && !vertexData[1].isEmpty()) {
            ...
            textureArray[currentVertexPointer * 2 + 1] = 0;
        }

        // Process normal (if present)
        if (vertexData.length > 2) {
            ...
            // Default normals if none provided
